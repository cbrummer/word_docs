...
jQuery('#filters input[type="radio"]').change(function(){
 var $this = jQuery(this),
  ...
  value = $this.val();
 state[name] = value;
 jQuery("#adcProviderSelect").select2("val", "*");
 state.prov = '';
 jQuery.bbq.pushState(state);
 $container.isotope({ filter: get_filter() });
});
...
jQuery("#adc-location-select").change(function() {
  jQuery("input[name='loc']:first").prop('checked', true);
  jQuery("#adcProviderSelect").select2("val", "*");
  state.prov = '';
  jQuery.bbq.pushState(state);
});
jQuery("input[name='loc']").change(function() {
  jQuery("#adc-location-select").val('*');
  jQuery("#adcProviderSelect").select2("val", "*");
  state.location = '';
  state.prov = '';
  jQuery.bbq.pushState(state);
});
jQuery("#adcProviderSelect").change(function() {
	 var state = jQuery.extend({}, defaultState, {prov: jQuery("#adcProviderSelect").val() });
	 jQuery.bbq.pushState(state);
});
...
jQuery('#adc-clear-filters').click(function () {
 ...
 jQuery(".multiselect input[name='loc']:first").prop('checked', true);
...
var noResultsCheck = function($element) {
    var noResults = jQuery('.adc-no-results');
    var numItems = $element.find('.adc-provider:not(.isotope-hidden)').length;
    if (numItems === 0) {
        noResults.show();
    } else {
        noResults.hide();
...
jQuery(window).bind('hashchange', function(e) {
 ...
 if (newState.loc != state.loc) {
  ...
  state.prov = '';
  jQuery("input[name='loc'][value='" + state.loc + "']").attr('checked', true);
 }
 ...
 if (newState.prov != state.prov) {
  state = newState
  jQuery("#adcProviderSelect").select2("val", state.prov);
 }
 
 $container.isotope({ filter: get_filter() });	
 noResultsCheck($container);
});
...
jQuery(window).smartresize(function(){
 $container = $container || jQuery('#adc-grid-content');
  $container.isotope({
    ...
    fitRows: { columnWidth: $container.width() / 4 }
  });
});
